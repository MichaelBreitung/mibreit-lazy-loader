...
var ElementSurrogate = /** @class */ (function () {
    ...
    ElementSurrogate.prototype.resize = function (horizontal) {
        if (horizontal) {
            lib_DomTools.addCssStyle(this._surrogate, "width", lib_DomTools.getElementDimension(this._surrogate).height *
    ...
    ElementSurrogate.prototype._createSurrogate = function (horizontal) {
        ...
        if (horizontal) {
            lib_DomTools.overwriteCssStyles(surrogate, "overflow: hidden; height: 100%; width: " + lib_DomTools.getElementDimension(this._element.getHtmlElement()).height *
                (this._element.getWidth()) / this._element.getHeight() + "px; flex-shrink:0;");
        }
        else {
            lib_DomTools.overwriteCssStyles(surrogate, "overflow: hidden; width: 100%; height: 0; padding-bottom: " + (this._element.getHeight() * 100) / this._element.getWidth() + "%;");
        }
        return surrogate;
    };
...
function createLazyLoaderFromElements(elements, config) {
    ...
    if (config.useSurrogate) {
        var _loop_1 = function (i) {
            var surrogate = new components_ElementSurrogate(elements[i], config.mode == ELazyMode.WINDOWED_SCROLL_HORIZONTAL);
    ...
    if (config.mode == ELazyMode.WINDOWED_SCROLL_HORIZONTAL) {
        ...
        lib_DomTools.addResizeEventListener(function () {
